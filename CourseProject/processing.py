...
def process_freqs(sht_files, signal_numbers, output_path, empirical_indexes=None):
    for sht_file in sht_files:
        ...
        for signal_number in signal_numbers:
            ...
            if empirical_indexes is not None and (
                    sawtooth_indexes[0] < empirical_indexes[0] or sawtooth_indexes[1] > empirical_indexes[1]):
        ...
        pyplot.title('Frequency of ' + os.path.basename(sht_file) + ' data')

        fig.savefig(os.path.join(output_path, os.path.splitext(os.path.basename(sht_file))[0] + '.png'))
